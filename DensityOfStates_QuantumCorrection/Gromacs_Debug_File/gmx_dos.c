/*
 *
 * This is a debug cpp source code mainly for DoS calculation
 *      gromacs--version 5.1.4
 * 
 * Before compile the Gromacs, please copy this file to 
 *         GROMACS/src/gromacs/gmxana/ 
 * then override the old one
 *
 ************************************************************************
 *
 *
...
//2017-9-12
//begin
    fp = xvgropen(opt2fn("-mvacf", NFILE, fnm), "XZ-Debug info",
                  "mi", "c-parameters", oenv);

    invNormalize = normalizeAutocorrelation ? 1.0/dos[VACF][0] : 1.0;
	
    fprintf(fp, "invNormalize = %10g \n", invNormalize);
    fprintf(fp, "     DIM     = %10d \n", DIM);
    fprintf(fp, "     gnx     = %10d \n", gnx);
    fprintf(fp, "  XX = %10d; YY = %10d; ZZ = %10d \n", XX, YY, ZZ);
    fprintf(fp, "    Natom    = %10d \n\n\n", Natom);

    fprintf(fp, "   i     j       mi        c1j     c1[i+XX]   c1[i+YY]  c1[i+ZZ]    dos[VACF]  dos[MVACF] \n\n");
	for (i = 0; (i < gnx); i += DIM)
    {
        mi = top.atoms.atom[index[i/DIM]].m;
        for (j = 0; (j < nframes/2); j++)
        {
            ...
            dos[VACF][j]  += c1j/Natom;
            dos[MVACF][j] += mi*c1j;
        	fprintf(fp, "%5d %5d %10g %10g %10g %10g %10g %10g %10g\n", i, j, mi, c1j, c1[i+XX][j], c1[i+YY][j], c1[i+ZZ][j], dos[VACF][j], dos[MVACF][j] );
        }
    }

	fprintf(fp,"\n\n The final dos[VACF] & dos[MVACF] results: \n\n");
    for (j = 0; (j < nframes/2); j++)
   {
		fprintf(fp, "%10d %10g %10g\n", j, dos[VACF][j], dos[MVACF][j] );
    }
    xvgrclose(fp);
//debug-done
    ...
    //2017-08-24
    //begin
    fp = xvgropen(opt2fn("-vacf", NFILE, fnm), "Mass-weighted vs itself-fft",
                  "dos[MVACF]", "dos[DOS]", oenv);
    for ( j=0; (j< nframes/2); j++)
    {
        fprintf(fp, "%10g %10g\n", dos[MVACF][j], dos[DOS][j]);
    }
    xvgrclose(fp);
    //debug-done
    ...
    //2017-09-24
    //begin
    fp = xvgropen(opt2fn("-vacf", NFILE, fnm), "Mass-weighted-DoS after process",
                  "dos[DoS]", "dos[DoS-after-process]", oenv);
    
    fprintf(fp, "dt = %10g \n", dt);
    fprintf(fp, "beta = %10g \n", beta);
    fprintf(fp, "bfac = %10g \n",bfac);
    
    fprintf(fp,"State-Choose     j      nu[j]      dos[DOS][j] \n");
    ...
    for (j = 0; (j < nframes/4); j++)
    {
        ...
        if (bAbsolute)
        {
            ...
            fprintf(fp,"Absolute  %5d %10g %10g \n", j, nu[j],dos[DOS][j]);
        }
        else
        {
            dos[DOS][j] = bfac*dos[DOS][2*j];
            fprintf(fp,"Real      %5d %10g %10g \n", j, nu[j],dos[DOS][j]);
            
        }
    }
    fprintf(fp,"\n\n The total dos = square(dos_real) + square(dos_image) = %15g \n", dos2);
    xvgrclose(fp);
    //debug-done
    ...
    fprintf(fplog, "AMU = %g \n", AMU);
    fprintf(fplog, "NANO = %g \n", NANO);
    fprintf(fplog, "M_PI = %g \n", M_PI);
    fprintf(fplog, "BOLTZ = %g \n", BOLTZ);
    fprintf(fplog, "PLANCK = %g \n", PLANCK);
    ...
    fprintf(fp, "\n recip_fac = %g \n\n", recip_fac);
    fprintf(fp, "Equation: \n");
    fprintf(fp, "dos[DOS_DIFF][j]  = DoS0/(1+sqr(DoS0*M_PI*nu[j]/(6*f*Natom))) \n");
    fprintf(fp, "dos[DOS_SOLID][j] = dos[DOS][j]-dos[DOS_DIFF][j] \n\n");
    ...
    //2017-09-25
    //begin
    fp = xvgropen(opt2fn("-vacf", NFILE, fnm), "Final-DoS part",
                  "dos", "dos", oenv);
    fprintf(fp, "wCdiff = %10g \n", wCdiff);
    fprintf(fp, "wSdiff = (Shs/(3*BOLTZ)) = %10g \n", wSdiff);
    fprintf(fp, "wEdiff = %10g \n", wEdiff);
    fprintf(fp, "wAdiff = (wEdiff-wSdiff) = %10g \n", wAdiff);
    fprintf(fp, "beta = %10g \n", beta);
    fprintf(fp, "cP = (BOLTZ*evaluate_integral(dos[DOS_CP]) = %10g \n\n", cP);
    fprintf(fp, "Equation: \n");
    fprintf(fp, "dos[DOS_CP][j] = (dos[DOS_DIFF][j]*wCdiff + dos[DOS_SOLID][j]*wCsolid(nu[j], beta)) \n");
    fprintf(fp, "dos[DOS_S][j]  = (dos[DOS_DIFF][j]*wSdiff + dos[DOS_SOLID][j]*wSsolid(nu[j], beta)) \n");
    fprintf(fp, "dos[DOS_A][j]  = (dos[DOS_DIFF][j]*wAdiff + dos[DOS_SOLID][j]*wAsolid(nu[j], beta)) \n");
    fprintf(fp, "dos[DOS_E][j]  = (dos[DOS_DIFF][j]*wEdiff + dos[DOS_SOLID][j]*wEsolid(nu[j], beta)) \n\n");
    
    fprintf(fp, "  j  dos[DOS_DIFF][j] dos[DOS_SOLID][j] nu[j] wCsolid(nu[j],beta) wSsolid(nu[j],beta) wAsolid(nu[j],beta) wEsolid(nu[j],beta) dos[DOS_CP][j], dos[DOS_S][j] dos[DOS_A][j] dos[DOS_E][j] \n");
    
    for ( j=0; (j< nframes/4); j++)
    {
        fprintf(fp, "%5d %10g %10g %10g %10g %10g %10g %10g %10g %10g %10g %10g\n", j, dos[DOS_DIFF][j], dos[DOS_SOLID][j], nu[j], wCsolid(nu[j],beta), wSsolid(nu[j],beta), wAsolid(nu[j],beta), wEsolid(nu[j],beta), dos[DOS_CP][j], dos[DOS_S][j], dos[DOS_A][j], dos[DOS_E][j]);
    }
    xvgrclose(fp);
    //debug-done
    
    fprintf(fplog, "Heat capacity = 1000*cP/Nmol : \n");

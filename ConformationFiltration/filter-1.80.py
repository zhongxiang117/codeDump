...
FEATURES = [
    ...
    'version 1.80 : add argparse features',
]
...
class ReadFile:
    ...
    def __init__(self,file,ext=None):
        ...
        if ext is None:
            ...
            if self.ext is None or self.ext not in ['pdb','gro']:
                self.ext = 'txt'
        elif ext not in ['txt','pdb','gro']:
    ...
    def read_txt(self):
        ...
        for cnt,line in enumerate(profile):
            sub = line.strip()
            if len(sub) > 0 and sub[0] == '#': continue
                ...
                if len(mol) != 0:
                    ...
                    for t in mol:
                        ...
                        if len(atom) == 4:
                            ...
                            bo = False
                        if bo:
                            promol.append([atom[0],x,y,z])
                        else:
                            print('Wrong: line {:}: {:}'.format(nm,t[0]))
                            print('Note: whole sets are omitted...\n')
                            break
                    if len(promol) == 0: bo = False

                    if bo: prolist.append(promol)
...
class Filtration:
    ...
    def calc_mols_filter_list(self,bondlist,anglelist,tolerance):
        ...
        #   2) if the last two are bad, the will be both removed
        reflist = []
        while True:
...
class BulkProcess:
    ...
    def run(self):
        ...
        systemlist = self.get_datalist(self.datafilelist)

        sysndxlist = []
        if len(self.indexfilelist) != 0:
            print('Note: reading all index file inputs')
            sysndxlist = self.get_datalist(self.indexfilelist)
        ...
        if bo:
            self.filtration_on_self(systemlist)
    ...
    def file_print(self):
        ...
        fd = file_gen_new('system',fextend='txt',foriginal=False)
        self.kwargs['fname'] = fd
    ...
    def filtration_on_self(self,datalist):
